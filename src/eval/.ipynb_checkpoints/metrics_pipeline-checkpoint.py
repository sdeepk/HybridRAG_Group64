...
#
# Run:
#   python src/eval/run_pipeline.py
# ------------------------------------------------------------
...
def save_error_overall_bar(df, out_path):
    if "error_type" not in df.columns:
        print("[WARN] error_type column missing; skipping overall_error_distribution plot.")
        return
    ...
    if err.empty:
        ...
        plt.text(0.5, 0.5, "No errors found", ha="center", va="center", fontsize=14)
        plt.axis("off")
        plt.tight_layout()
        plt.savefig(out_path, dpi=200)
        plt.close()
...
def main():
    ...
    for q in questions:
        ...
        gt_text = q.get("answer") or ""
        gt_urls = q.get("source_urls", [])

        # prefer textual ground truth if present, else show URLs (URL-level GT is acceptable)
        ground_truth = gt_text.strip() if gt_text else " | ".join(gt_urls)
        ...
        rows.append({
            ...
            # Mandatory metric at per-question level (URL-level RR)
            "MRR": rr,
            ...
            "reciprocal_rank": rr,
            # time
            "Time (sec)": round(t2 - t0, 4),
        
            # optional debug columns (keep if you want)
            "category": category,
    ...
    # Save assignment-required results table
    results_table_cols = [
        "Question ID", "Question", "Ground Truth", "Generated Answer",
        "MRR", "precision@5", "Faithfulness", "Time (sec)"
    ]
    
    df[results_table_cols].to_csv(os.path.join(OUT_DIR, "results_table.csv"), index=False)
    df[results_table_cols].to_json(
        os.path.join(OUT_DIR, "results_table.json"),
        orient="records",
        indent=2,
        force_ascii=False
    )
